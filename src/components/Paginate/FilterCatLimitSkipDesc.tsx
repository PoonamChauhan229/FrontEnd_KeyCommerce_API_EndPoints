...
const FilterCatLimitSkipDesc: React.FC = () => {
  // Find the "Products" section in the sections array
  const productSection = apiData.sections.find((section) => section.sectionTitle === "Paginate");
  ...
  const getProductEndpoint = productSection.endpoints.find(
    (endpoint) => endpoint.title === "Filter Products by Category, Limit, Skip, and Sort by Price Descending"
  );
...
export default FilterCatLimitSkipDesc;
