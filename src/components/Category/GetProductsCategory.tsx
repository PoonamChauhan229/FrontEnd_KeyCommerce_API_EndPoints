...
const GetProductsCategory: React.FC = () => {
  // Find the "Products" section in the sections array
  const productSection = apiDataTyped.sections.find((section) => section.sectionTitle === "Category");
  ...
  const getProductEndpoint = productSection.endpoints.find(
    (endpoint) => endpoint.title === "Get Products by Category"
  );
